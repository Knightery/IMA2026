...
- Assumed 20,000 units sold per candidate city (as a comparable city-selection scenario).
- Cost model: acquisition (`$0.20 * 20,000`) + shipping (`$1,500 + $50 per 100 miles`).
- Ranked cities by expected profit and selected top five.
...
Rationale: these cities maximize expected profit by combining higher local price support (higher wholesale revenue) with manageable logistics cost.

Supporting CSVs: `q5_june2025_city_prices.csv`, `q5_city_profitability_base.csv`, `q5_top5_cities.csv`.